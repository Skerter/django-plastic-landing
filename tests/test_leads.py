...
def test_email_notification_sent(mailoutbox):
    ...
    site_settings.email_notifications = "notifications@demoplast.test"
    site_settings.save()
    lead = LeadFactory()
    ...
    msg = mailoutbox[0]
    assert msg.to == ["notifications@demoplast.test"]
...
def test_lead_form_post_sends_email(client, mailoutbox):
    ...
    assert len(mailoutbox) == 1
    assert mailoutbox[0].to == ["notifications@demoplast.test"]
