...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('code', help='Authorization code из карточки интеграции amoCRM')

    def handle(self, *args, **options):
        from apps.leads.models import AmoCRMAuth

        if not (settings.AMOCRM_BASE_URL and settings.AMOCRM_CLIENT_ID and settings.AMOCRM_CLIENT_SECRET):
            raise CommandError('Не заданы AMOCRM_BASE_URL / CLIENT_ID / CLIENT_SECRET в env.')
